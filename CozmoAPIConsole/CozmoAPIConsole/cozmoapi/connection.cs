...
namespace Anki
{
    namespace Cozmo
    {
        public class SdkConnection
        {
            ...
            // adb info
            private string _adb_directory = null;
            private string _adb_exe = null;
            ...
            public SdkConnection(string adbpath, string ip, int port, bool verboseLogging = false)
            {
                // process adb location
                processADB(adbpath);
            ...
            private void processADB(string adbpath)
            {
                // if the given path does not end with a \
                if (!adbpath.EndsWith("\\"))
                {
                    // add one
                    adbpath += "\\";
                    // set as adb directory
                    _adb_directory = adbpath;
                }
                // add the adb executable
                _adb_exe = _adb_directory + "adb.exe";
                // if this is not the correct path to the adb executable
                if (!File.Exists(_adb_exe))
                {
                    string foundpath = null;
                    // search for the path to the adb executable
                    ...
                    // if the path to the adb executable was not found
                    if (foundpath == null)
                    {
                        throw new ArgumentException("Could not find adb.exe on the path supplied: " + adbpath);
                    }
                    else
                    {
                        _adb_directory = foundpath;
                        _adb_exe = _adb_directory + "adb.exe";
                        Console.WriteLine("Found adb here {0}", _adb_exe);
                    }
                }
            }
            ...
            public void Connect()
            {
                Console.WriteLine("Forwarding to bridge");

                ExeADB("forward tcp:{0} tcp:{0}", _port);
            ...
            {
                ExeADB("forward --remove tcp:{0}", _port);
                if (_open)
                {
                    _open = false;
                ...
                {
                    Console.WriteLine("No robot to disconnect from...");
                }
            }

            private void ExeADB(string parameters, params object[] data)
            {
                ProcessStartInfo info = new ProcessStartInfo(_adb_exe);
